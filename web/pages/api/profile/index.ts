...
async function profile(req: NextApiRequest, res: NextApiResponse<ApiResponse>) {
  try {
    ...
    const { rows } = await client.query(
      "SELECT id, email, first_name, middle_name, last_name, banner_image_url, is_premium, major, avatar_url, bio, username, expected_grad_date, created_at FROM users WHERE email = $1",
      [req.session.user?.email]
    );
